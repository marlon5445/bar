...
<div class="page-header">
    <div class="page-header-left">
        ...
        <p class="page-subtitle">Administra los clientes y sus límites de crédito</p>
    </div>
    <div class="page-header-actions">
        <a href="<?= site_url('clientes/crear'); ?>" class="btn-primary">
...
<div class="toolbar-container" style="margin-bottom: 1.5rem; display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
    <div class="search-wrapper" style="position: relative; flex: 1; min-width: 300px;">
        ...
        <input type="text" id="buscarCliente" placeholder="Buscar cliente por nombre, teléfono o estado..." 
    ...
    <div id="contadorResultados" class="text-muted" style="font-size: 0.9rem; font-weight: 500;">
        <?php if (!empty($clientes)): ?>
            <?= count($clientes); ?> clientes en total
        <?php endif; ?>
    </div>
</div>
                ...
                <?php if (!empty($clientes)): ?>
                    <?php foreach ($clientes as $c): ?>
                        <tr>
                            ...
                            <td><?= esc($c['telefono']) ?: '<span class="text-muted">No registrado</span>'; ?></td>
                            <td><span class="text-muted">S/</span> <?= number_format($c['limite_credito'], 2); ?></td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="<?= site_url('clientes/editar/' . $c['id']); ?>" class="btn-icon-only btn-edit" title="Editar">
                                        ✏️
                                    </a>
                                    
                                    <form action="<?= site_url('clientes/cambiar-estado/' . $c['id']); ?>" method="POST" style="display:inline;" class="form-dinamico-estado">
                                        <?= csrf_field(); ?>
...
$(document).ready(function() {
    ...
    $searchInput.on('input', function() {
        ...
        if (searchTerm === "") {
            ...
            $tableRows.each(function() {
                ...
                const telefono = $row.find('td:nth-child(2)').text().toLowerCase();
                const estado = $row.find('td:nth-child(4)').text().toLowerCase();

                if (nombre.includes(searchTerm) || telefono.includes(searchTerm) || estado.includes(searchTerm)) {
                    $row.show();
                    visibleCount++;
                } else {
                    $row.hide();
            ...
            $tbody.append(`
                <tr class="no-results-row">
                    <td colspan="6" class="text-center py-4">
                        <div class="empty-state">
                            <span class="empty-icon">🔍</span>
                            <p>No se encontraron clientes que coincidan con "${escapeHtml(searchTerm)}"</p>
                        </div>
                    </td>
                </tr>
            `);
        ...
        // Actualizar contador
        if (searchTerm === "") {
            $contador.text(`${totalRegistros} clientes en total`);
        } else {
            $contador.text(`${visibleCount} de ${totalRegistros} clientes encontrados`);
        }
    });
    ...
    $('.btn-trigger-dinamico').on('click', function() {
        ...
        if (title === 'Activar') {
            $modalTitle.text('¿Activar Cliente?');
            $modalIcon.text('✅').css({ 'background': 'rgba(16, 185, 129, 0.15)', 'color': '#10b981' });
            $modalMsg.text('¿Desea activar nuevamente al cliente');
            $btnConfirm.text('Sí, Activar').css('background', 'var(--success)');
        } else if (title === 'Desactivar') {
            $modalTitle.text('¿Desactivar Cliente?');
            $modalIcon.text('🚫').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('¿Desea desactivar al cliente');
            $modalWarning.text('Este cliente tiene historial en el sistema y no puede eliminarse definitivamente.').show().css('color', 'var(--text-muted)');
            $btnConfirm.text('Sí, Desactivar').css('background', '#dc2626');
        } else if (title === 'Eliminar') {
            $modalTitle.text('¿Eliminar Cliente?');
            $modalIcon.text('🗑️').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('Este cliente se eliminará de manera definitiva. ¿Desea eliminar al cliente');
