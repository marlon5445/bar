...
<div class="page-header">
    <div class="page-header-left">
        ...
        <p class="page-subtitle">Administra las categorías de productos del bar</p>
    </div>
    <div class="page-header-actions">
        <a href="<?= site_url('categorias/crear'); ?>" class="btn-primary">
            <span class="btn-icon">+</span> Nueva Categoría
        </a>
    </div>
</div>
...
<div class="toolbar-container" style="margin-bottom: 1.5rem; display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
    <div class="search-wrapper" style="position: relative; flex: 1; min-width: 300px;">
        ...
        <input type="text" id="buscarCategoria" placeholder="Buscar categoría por nombre, descripción o estado..." 
            ...
            <tbody>
                <?php if (!empty($categorias)): ?>
                    <?php foreach ($categorias as $c): ?>
                        <tr>
                            <td><span class="text-muted">#</span><?= $c['id']; ?></td>
                            <td><strong><?= esc($c['nombre']); ?></strong></td>
                            <td>
                                <div class="text-truncate" style="max-width: 250px;" title="<?= esc($c['descripcion']); ?>">
                                    <?= esc($c['descripcion']) ?: '<span class="text-muted">Sin descripción</span>'; ?>
                                </div>
                            </td>
                            <td>
                                <span class="badge-status <?= $c['estado'] === 'ACTIVO' ? 'status-activo' : 'status-inactivo'; ?>">
                                    <?= esc($c['estado']); ?>
                                </span>
                            </td>
                            <td><span class="text-muted" style="font-size: 0.85rem;"><?= date('d/m/Y H:i', strtotime($c['fecha_creacion'])); ?></span></td>
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="<?= site_url('categorias/editar/' . $c['id']); ?>" class="btn-icon-only btn-edit" title="Editar">
                                        ✏️
                                    </a>
                                    
                                    <form action="<?= site_url('categorias/cambiar-estado/' . $c['id']); ?>" method="POST" style="display:inline;" class="form-dinamico-estado">
                                        <?= csrf_field(); ?>
...
$(document).ready(function() {
    ...
    $searchInput.on('input', function() {
        ...
        if (searchTerm === "") {
            ...
            $tableRows.each(function() {
                ...
                const descripcion = $row.find('td:nth-child(3)').text().toLowerCase();
                const estado = $row.find('td:nth-child(4)').text().toLowerCase();

                if (nombre.includes(searchTerm) || descripcion.includes(searchTerm) || estado.includes(searchTerm)) {
                    $row.show();
                    visibleCount++;
                } else {
                    $row.hide();
            ...
            $tbody.append(`
                <tr class="no-results-row">
                    <td colspan="6" class="text-center py-4">
                        <div class="empty-state">
                            <span class="empty-icon">🔍</span>
                            <p>No se encontraron categorías que coincidan con "${escapeHtml(searchTerm)}"</p>
                        </div>
                    </td>
                </tr>
            `);
        ...
        // Actualizar contador
        if (searchTerm === "") {
            $contador.text(`${totalRegistros} categorías en total`);
        } else {
            $contador.text(`${visibleCount} de ${totalRegistros} categorías encontradas`);
        }
    });
    ...
    $('.btn-trigger-dinamico').on('click', function() {
        ...
        if (title === 'Activar') {
            $modalTitle.text('¿Activar Categoría?');
            $modalIcon.text('✅').css({ 'background': 'rgba(16, 185, 129, 0.15)', 'color': '#10b981' });
            $modalMsg.text('¿Desea activar nuevamente la categoría');
            $btnConfirm.text('Sí, Activar').css('background', 'var(--success)');
        } else if (title === 'Desactivar') {
            $modalTitle.text('¿Desactivar Categoría?');
            $modalIcon.text('🚫').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('¿Desea desactivar la categoría');
            $modalWarning.text('Esta categoría tiene productos asociados y no puede eliminarse definitivamente.').show().css('color', 'var(--text-muted)');
            $btnConfirm.text('Sí, Desactivar').css('background', '#dc2626');
        } else if (title === 'Eliminar') {
            $modalTitle.text('¿Eliminar Categoría?');
            $modalIcon.text('🗑️').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('Esta categoría se eliminará de manera definitiva. ¿Desea eliminar la categoría');
