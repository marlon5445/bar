...
<div class="page-header">
    <div class="page-header-left">
        <h1 class="page-title">👨‍🍳 Gestión de Meseros</h1>
        <p class="page-subtitle">Administra el personal de atención del bar</p>
    </div>
    <div class="page-header-actions">
        <a href="<?= site_url('meseros/crear'); ?>" class="btn-primary">
            <span class="btn-icon">+</span> Nuevo Mesero
        </a>
    </div>
</div>
...
<div class="toolbar-container" style="margin-bottom: 1.5rem; display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
    <div class="search-wrapper" style="position: relative; flex: 1; min-width: 300px;">
        ...
        <input type="text" id="buscarMesero" placeholder="Buscar mesero por nombre, teléfono o estado..." 
    ...
    <div id="contadorResultados" class="text-muted" style="font-size: 0.9rem; font-weight: 500;">
        <?php if (!empty($meseros)): ?>
            <?= count($meseros); ?> meseros en total
        <?php endif; ?>
    </div>
</div>
                ...
                <?php if (!empty($meseros)): ?>
                    <?php foreach ($meseros as $m): ?>
                        <tr>
                            ...
                            <td><?= esc($m['telefono']) ?: '<span class="text-muted">No registrado</span>'; ?></td>
                            <td>
                                <span class="badge-status <?= $m['estado'] === 'ACTIVO' ? 'status-activo' : 'status-inactivo'; ?>">
                                    <?= esc($m['estado']); ?>
                                </span>
                            </td>
                            <td><span class="text-muted" style="font-size: 0.85rem;"><?= date('d/m/Y H:i', strtotime($m['fecha_creacion'])); ?></span></td>
                            <td class="text-center">
                                <div class="btn-group">
                                    <a href="<?= site_url('meseros/editar/' . $m['id']); ?>" class="btn-icon-only btn-edit" title="Editar">
                                        ✏️
                                    </a>
                                    
                                    <form action="<?= site_url('meseros/cambiar-estado/' . $m['id']); ?>" method="POST" style="display:inline;" class="form-dinamico-estado">
                                        <?= csrf_field(); ?>
                    ...
                    <tr>
                        <td colspan="5" class="text-center py-4">
                            <div class="empty-state">
                                ...
                                <p>No existen meseros registrados.</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
...
$(document).ready(function() {
    ...
    $searchInput.on('input', function() {
            ...
            $tbody.append(`
                <tr class="no-results-row">
                    <td colspan="5" class="text-center py-4">
                        <div class="empty-state">
                            <span class="empty-icon">🔍</span>
                            <p>No se encontraron meseros que coincidan con "${escapeHtml(searchTerm)}"</p>
                        </div>
                    </td>
                </tr>
            `);
        ...
        // Actualizar contador
        if (searchTerm === "") {
            $contador.text(`${totalRegistros} meseros en total`);
        } else {
            $contador.text(`${visibleCount} de ${totalRegistros} meseros encontrados`);
        }
    });
    ...
    $('.btn-trigger-dinamico').on('click', function() {
        ...
        const relacion = $btn.data('relacion');
        ...
        if (title === 'Activar') {
            $modalTitle.text('¿Activar Mesero?');
            $modalIcon.text('✅').css({ 'background': 'rgba(16, 185, 129, 0.15)', 'color': '#10b981' });
            $modalMsg.text('¿Desea activar nuevamente al mesero');
            $btnConfirm.text('Sí, Activar').css('background', 'var(--success)');
        } else if (title === 'Desactivar') {
            $modalTitle.text('¿Desactivar Mesero?');
            $modalIcon.text('🚫').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('¿Desea desactivar al mesero');
            $modalWarning.text('Este mesero tiene ventas registradas y no puede eliminarse definitivamente.').show().css('color', 'var(--text-muted)');
            $btnConfirm.text('Sí, Desactivar').css('background', '#dc2626');
        } else if (title === 'Eliminar') {
            $modalTitle.text('¿Eliminar Mesero?');
            $modalIcon.text('🗑️').css({ 'background': 'rgba(239, 68, 68, 0.15)', 'color': '#ef4444' });
            $modalMsg.text('Este mesero se eliminará de manera definitiva. ¿Está seguro de eliminar al mesero');
